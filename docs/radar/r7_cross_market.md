...
**Component timing authority (Correction A, strengthened in Correction B):** timing validity is **component-local and causal**. Every observation that causally contributes to the component price participates — both endpoints AND the FX observation whenever FX normalization was actually applied (FX age 500 s inside a 600 s staleness window but outside a 120 s skew window makes the FX-dependent component timing-invalid). A component is timing-valid only if (1) no dependency timestamp is in the future relative to `as_of`, (2) every dependency is fresh within `policy.stale_layer_seconds`, and (3) the oldest-to-newest skew across ALL dependencies is within `policy.max_layer_skew_seconds`. Timing-invalid components are retained as evidence (with `timingDependencyObservations`, `timingOldest`, `timingNewest`, `timingSkewSeconds` serialized for audit) but excluded from attribution: material differences with invalid timing yield `ATTRIBUTION_UNAVAILABLE` (with `TIMING_SKEW`/`STALE_LAYER` reasons), never a normal material attribution. `STALE_LAYER` remains distinguishable from `TIMING_SKEW` and `TIMING_UNRESOLVED`; future observations are never normalized into fresh data.
...
**Upstream R3 lineage is validated by the contract itself (Correction B):** when `upstreamEvidence["r3LiquidityEvidence"]` or `sourceDigests["r3LiquidityDigest"]` is supplied, the complete pair is required and the canonical-JSON SHA-256 of the embedded evidence must equal the supplied digest — otherwise construction fails typed closed (`CROSS_MARKET_EVIDENCE_MISMATCH`). A valid outer `r7SnapshotDigest` never legitimizes internally inconsistent upstream lineage.
